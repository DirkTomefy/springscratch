...
public class FrontServletContextListener implements ServletContextListener {
    ...
    public void contextInitialized(ServletContextEvent sce) {
        ...
        try {
            ...
            if (providerClassName == null || providerClassName.isBlank()) {
                ...
                if (!BeanProvider.class.isAssignableFrom(providerClass)) {
                    throw new RuntimeException(providerClassName + " doit hériter de " + BeanProvider.class.getName());
                }
                Constructor<?> constructor = providerClass.getConstructor(ServletContext.class);
                beanProvider = (BeanProvider) constructor.newInstance(context);
            }
        } catch (Exception e) {
            throw new RuntimeException("Impossible d'initialiser le BeanProvider.", e);
        }
        ...
        if (configClassName != null && !configClassName.isBlank()) {
            try {
                Class<?> annotationConfigClass = Class.forName("org.springframework.context.annotation.AnnotationConfigApplicationContext");
                Class<?> configClass = Class.forName(configClassName);

                Constructor<?> constructor = annotationConfigClass.getConstructor(Class[].class);
                springContext = constructor.newInstance((Object) new Class<?>[]{configClass});

            } catch (Exception e) {
                throw new RuntimeException("Impossible d'initialiser le contexte Spring.", e);
            }
        }

        
        FrontServletParam urlProcessor = new FrontServletParam(beanProvider);
        urlProcessor.setExternalContext(springContext);
